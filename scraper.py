...
class AuctionScraper:
    ...
    async def wait_for_element(self, selector, timeout=40000):
        """Wait for element to appear (like WebDriverWait)"""
        try:
    ...
    def run(self, site, url, start_page, end_page):
        try:
            ...
            if site == "HiBid": 
                ...
                loop.run_until_complete(self.scrape_hibid_async(url, start_page, end_page))
            elif site == "BiddingKings": 
                loop = asyncio.get_event_loop()
                loop.run_until_complete(self.scrape_biddingkings_async(url, start_page, end_page))
            elif site == "BidLlama": 
                loop = asyncio.get_event_loop()
                loop.run_until_complete(self.scrape_bidllama_async(url, start_page, end_page))
            elif site == "Nellis": 
                ...
                loop = asyncio.get_event_loop()
                loop.run_until_complete(self.scrape_macbid_async(url, start_page, end_page))
            elif site == "A-Stock":
                ...
                loop = asyncio.get_event_loop()
                loop.run_until_complete(self.scrape_vista_async(url, start_page, end_page))
            elif site == "BidSoflo":
                loop = asyncio.get_event_loop()
                loop.run_until_complete(self.scrape_bidsoflo_async(url, start_page, end_page))
            elif site == "BidAuctionDepot":
                loop = asyncio.get_event_loop()
                loop.run_until_complete(self.scrape_bidauctiondepot_async(url, start_page, end_page))
        except Exception as e:
    ...
    # AI-Powered Scrapers with Pyppeteer
    async def scrape_hibid_async(self, url, start_page, end_page):
        ...
        while self.running and (end_page == 0 or page <= end_page):
            ...
            await self.navigate_to(current_url)
            self.ui['metrics']['pages'].metric("Pages Scraped", page)
            
            element_found = await self.wait_for_element("h2.lot-title", timeout=40000)
    ...
    async def scrape_biddingkings_async(self, url, start_page, end_page):
        ...
        while self.running and (end_page == 0 or page <= end_page):
            ...
            element_found = await self.wait_for_element("div[class*='lot-repeater-index']", timeout=40000)
            if not element_found:
                self.ui['status'].success("No more pages found. Scraping complete.")
                break
            ...
            for i, p in enumerate(products, 1):
                ...
                if link_tag and img_tag:
                    ...
                    await self.navigate_to(product_url)
                    product_html = await self.get_page_content()
    ...
    async def scrape_bidllama_async(self, url, start_page, end_page):
        ...
        while self.running and (end_page == 0 or page <= end_page):
            ...
            element_found = await self.wait_for_element("p.item-lot-number", timeout=40000)
            if not element_found:
                self.ui['status'].success("No more pages found. Scraping complete.")
                break
...
async def scrape_bidauctiondepot_async(self, url, start_page, end_page):
    if True:
        ...
        while self.running and flag and (end_page == 0 or page <= end_page):
            try:
                ...
                element_found = await self.wait_for_element('div[class^="card grid-card a gallery auction"]', timeout=25000)
                if element_found:
                    self.ui['status'].info("Product cards loaded successfully")
                else:
                    self.ui['status'].error("Timeout waiting for products")
                ...
                try:
                    ...
                    if html_check:
                        await self.click_element("a[aria-label='Go to next page']")
                        page += 1
                        await asyncio.sleep(3)
    ...
    # Non-browser scrapers (use requests - no changes needed)
