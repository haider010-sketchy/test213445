        ...
        if result.data:
            ...
            for item in result.data:
                stored_data.append({
                    ...
                    'Retail': item.get('retail_price', ''),  # ADD RETAIL PRICE
                    'Fetch_Success': True,
                    'Error': None,
...
def get_amazon_product_details(asin, log_queue, processing_id, total_count, retail_price=None):
    ...
    return product_detailsASIN {asin}: Zyte API Status Code: {api_response.status_code} on attempt {attempt+1}'))
            ...
            if retail_col:
                ...
                if not asin_row.empty:
                    ...
                    if pd.notna(price_value):
                        retail_price = str(price_value).replace('
...
def display_fullscreen_grid(df, search_term=None, min_price=None, max_price=None, sort_by=None):
    ...
    if retail_col:
        ...
        try:
            ...
            filtered_df[f'{retail_col}_numeric'] = filtered_df[retail_col].astype(str).str.replace('
